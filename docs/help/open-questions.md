...
??? info "What changed at the 2026-09-24 review"
    ...
    - **Speculative decoding moved to approved drafter lists** per benchmark (§2.9.4). Approved heads are published for the agentic benchmarks only, which is **C10**.
...
| **B16** | **The `submission-checker` command isn't installed** | The CLI's README documents a `submission-checker` command with `check` and `regions` subcommands, but `pyproject.toml` has only the `endpoints-submission-cli` entry point (removed in commit `08e8e2a`, still absent at `v1.0.1.0`). Checking is `endpoints-submission-cli check-submission`; `regions` has no installed command. | `check-submission` for checks, and `python -c "from submission_checker.cli import main; main()" regions …` for boundaries |
...
| **C10** | **Approved drafter lists** | [§2.9.4][rules-2.9.4] allows speculative decoding only with a drafter on the benchmark's published list. The reference repository lists approved heads for the agentic benchmarks only, in its agentic example README; there is none for the legacy benchmarks. The checker still ships an empty list, and a drafter approved now can only be used two cohorts later. Until the checker picks up a list, it rejects any point that uses speculative decoding |
...
| **Speculative decoding** | [§2.9.4][rules-2.9.4]'s approved-list model is new for v1.0 and marked tentative. Approved heads exist for the agentic benchmarks only — see **C10** |
