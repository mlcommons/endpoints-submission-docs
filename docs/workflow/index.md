...
flowchart TB
    subgraph setup ["Get set up"]
        direction LR
        R[1 Get access] --> I[2 Install] --> P[3 Plan curve]
    end
    ...
    subgraph submit ["Register, check and submit"]
        direction LR
        G[6 Register runs] --> V[7 Validate] --> S[8 Submit] --> A[9 After you submit]
        V -.->|"errors: fix and register again"| G
    end
...
| [1. Get access and an API key](register.md) | A [PRISM API key](../understand/eligibility/prism-api-key.md) in `mlc_…` format |
...
| [7. Validate locally](validate.md) | A clean checker report |
